function P = q2pixel_temp(q_modulus, azimangle, detector_angle, SDD, psize, wl, edensity, beta, ai)
% P=q2pixel_temp(q_modulus, azimangle, detector_angle, SDD, psize, wl)
% q2pixel(q, azimangle, ...)
% q2pixel([qxy, qz], [], ...)
% q2pixel([qx, qy, qz], [], ...)
...
if size(q_modulus,2)==2 % for a 2D powder
    ...
    %ac = criticalang(edensity, wl);
    ai = ai*pi/180;
    %ac = deg2rad(ac);
    
    % q2z to af
    d1 = 1/(2*pi)*r0*wl^2*edensity;
    n = 1-d1+sqrt(-1)*beta;
            
    qxy = q_modulus(:,1); qz = q_modulus(:,2);
    atf = acos(sqrt(n^2-(qz/k0+sqrt(n^2-cos(ai)^2))^2));
    arf = acos(sqrt(n^2+(qz/k0+sqrt(n^2-cos(ai)^2))^2));
    tthtf = acos((cos(ai)^2+cos(atf)^2-(qxy/k0).^2)./(2*cos(ai)*cos(atf)));
    tthrf = acos((cos(ai)^2+cos(arf)^2-(qxy/k0).^2)./(2*cos(ai)*cos(arf)));
    P{1} = angle2pixel(tthtf, atf, 0, ai, detector_angle, SDD, psize);
    P{2} = angle2pixel(tthrf, arf, 0, -ai, detector_angle, SDD, psize);
    % Check the Bragg condition
end
