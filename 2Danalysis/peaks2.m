...
%
% Updated by Byeongdu Lee 1/21/2025
% Mask is added.
% EdgeDistance is added
% ThresholdNeighborDistance is added. So, looking for mean intensity of
% pixels that are ThresholdNeighborDistance away from the peak position.
% ThresholdType : scale or difference.
% When checking the threshold, it check whether scaled by the Threshold or
% differ by the Threshold.
...
isparm=find(cellfun(@(x)isequal(x,'Mask'),params),1);
if ~isempty(isparm)
    mask = vals{isparm};
else
    mask = [];
...
% 2. Threshold - peak must be greater than its neighbours by this value.
isparm=find(cellfun(@(x)isequal(x,'EdgeDistance'),params),1);
if ~isempty(isparm)
    edgeDist = vals{isparm};

    if isempty(mask)
        [row_y,col_x]=ind2sub(size(data),locs);
        t = row_y <= edgeDist | row_y >= size(data,1)-edgeDist | col_x >= size(data,2)-edgeDist | col_x <= edgeDist;
        locs(t) = [];
        pks(t) = [];
    else
        ...
        delta_y=[-1 -1 -1 0 0 +1 +1 +1]*edgeDist;
        delta_x=[-1 0 +1 -1 +1 -1 0 +1]*edgeDist;
        ...
        t = neighbor_locs_y <= edgeDist | neighbor_locs_y >= size(data,1)-edgeDist | neighbor_locs_x >= size(data,2)-edgeDist | neighbor_locs_x <= edgeDist;
        clear neighbor_locs_y neighbor_locs_x

        % So we have neighbor values by now. Are our peaks higher than those by
        % the set Threshold value? 
        suitable = ~any(isnan(mask(neighbor_locs)),2);
        suitable(t) = false;
        
        clear t neighbor_locs
...
% 2. Threshold - peak must be greater than its neighbours by this value.
isparm=find(cellfun(@(x)isequal(x,'ThresholdNeighborDistance'),params),1);
if ~isempty(isparm)
    pix_away = vals{isparm};
else
    pix_away = 15;
end
% 2. Threshold - peak must be greater than its neighbours by this value.
isparm=find(cellfun(@(x)isequal(x,'ThresholdType'),params),1);
if ~isempty(isparm)
    threshold_sign = vals{isparm};
else
    threshold_sign = '*';
end
...
if ~isempty(isparm)
    ...
    % (+1,-1)  (+1,0)  (+1,+1)
    % ...turned into vectors disregarding the (0,0), the center data point:
    delta_y=[-1 -1 -1 0 0 +1 +1 +1]*pix_away;
    delta_x=[-1 0 +1 -1 +1 -1 0 +1]*pix_away;
    % Let's add these deltas to the detected peak positions to get the
    % coordinates of their immediate neighbors:
    neighbor_locs_y=row_y+delta_y;
    neighbor_locs_x=col_x+delta_x; clear row_y col_x delta_x delta_y
    ...
    % So we have neighbor values by now. Are our peaks higher than those by
    % the set Threshold value? 
    if strcmp(threshold_sign, '*')
        suitable=(data(locs)*vals{isparm}>=nanmean(data(neighbor_locs),2));
    else
        suitable=(data(locs)-vals{isparm}>=nanmean(data(neighbor_locs),2));
    end
    ...
    % neighbor as well:
    suitable(any(data(neighbor_locs)==data(locs),2))=true;
